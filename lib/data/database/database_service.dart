...
class DatabaseService {
  ...
  Future<Map<String, dynamic>?> get(String table, String id) async {
    Map<String, dynamic>? value = await stringMapStoreFactory
        .store('Store $table')
        .record(id)
        .get(db);
    return value;
  ...
  Future<List<Map<String, dynamic>>?> getAll(
    ...
    var snapshots = await stringMapStoreFactory
        .store('Store $table')
  ...
  Future<void> insertAll(
    ...
    Future execute(DatabaseClient txn) async {
      for (Map<String, dynamic> record in values) {
        ...
        if (id != null) {
          await store.record(id.toString()).put(txn, record);
        } else {
  ...
  Future<void> delete(String table, String id, {DatabaseClient? txn}) async {
    await stringMapStoreFactory
        .store('Store $table')
        .record(id)
        .delete(txn ?? db);
  ...
  Future<void> executeTransaction(
    FutureOr Function(DatabaseClient) transaction,
  ) async {
    try {
      await mutex.acquire();
